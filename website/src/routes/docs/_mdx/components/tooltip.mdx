...
`TooltipGroup` 组件可用于同步多个工具提示的开合延迟。
      ...
      {[
        ...
        <Tooltip key={placement} placement={placement}>
          <TooltipTrigger
            ...
            className="flex h-12 w-12 cursor-default items-center justify-center rounded-md bg-bg-muted text-center text-sm leading-tight"
          >
            {name}
          </TooltipTrigger>
          <TooltipContent>
...
::PropsTable{file=tooltip/tooltip-group.tsx}
